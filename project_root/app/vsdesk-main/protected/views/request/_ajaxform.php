...
if (isset($fields) AND $fields) {
    foreach ($fields as $field) {
        ...
        if ($field->type == 'textFieldRow') {
            ...
            <input class="span12" maxlength="500" name="Request[' . $field->id . ']" id="Request_' . $field->id . '" type="text" value="'.$value.'"/></div>';
        }
        if ($field->type == 'date') {
        ...
        if ($field->type == 'toggle') {
            ...
            $this->widget(
                ...
                array('name' => "Request[" . $field->id . "]", 'value' => $value?$value:0)
            );
            echo '</div>';
            echo "
            <script>
            $('#wrapper-Request_" . $field->id . "').toggleButtons({'width':100,'height':25,'animated':true,'label':{'enabled':'Да','disabled':'Нет'},'style':{'enabled':'primary'}});
            </script>
            ";
        }
        if ($field->type == 'ruler') {
