...
Yii::app()->clientScript->registerScript('search', "
    ...
    $('.search-form form').submit(function(){
        $.fn.yiiGridView.update('request-grid-full', {
            data: $(this).serialize()
        });
        return false;
    });
    ...
    <div class="box">
      ...
      <?php if (Yii::app()->user->checkAccess('batchAssignRequest')): ?>
        <li><a href="javascript:void(0);" id="setuser"><i class="fa-solid fa-user fa-xl"
            title="<?php echo Yii::t('main-ui', 'Assign to user'); ?>"></i> </a>
        </li>
    <?php endif; ?>
    <?php if (Yii::app()->user->checkAccess('batchAssignRequest')): ?>
   ...
   <?php if (Yii::app()->user->checkAccess('batchUpdateRequest')): ?>
       ...
       ?>
      <li><a href="javascript:void(0);" id="close" data-need_comment="<?= $closeNeedComment ?>" data-need_rating="<?= $closeNeedRating ?>"><i class="fa-solid fa-circle-check fa-xl"
                                                      title="<?php echo Yii::t('main-ui', 'Close selected'); ?>"></i> </a></li>
      <?php endif; ?>
 ...
 <?php if (Yii::app()->session['requestStopTimer'] == 1): ?>
    <li>
        <a href="javascript:void(0);" onclick="$.ajax({data:{stoptimer:'stop'}, success: function(){location.reload();}});" id="stop-timer">
            <i class="fa-solid fa-hourglass-end fa-xl" title="<?php echo Yii::t('main-ui', 'Stop autoupdate timer'); ?>"></i> </a>
        </li>
    <?php else: ?>
        <li>
            <a href="javascript:void(0);" onclick="$.ajax({data:{stoptimer:'start'}, success: function(){location.reload();}});" id="start-timer">
                <i class="fa-solid fa-hourglass-start fa-xl" title="<?php echo Yii::t('main-ui', 'Start autoupdate timer'); ?>"></i> </a>
            </li>
        <?php endif; ?>
        <?php if (Yii::app()->session['requestlastactivity'] == 1): ?>
            <li>
                <a href="javascript:void(0);" onclick="$.ajax({data:{lastactivity:'stop'}, success: function(){location.reload();}});" id="stop-last">
                    <i class="fa-solid fa-arrow-down-9-1 fa-xl" title="<?php echo Yii::t('main-ui', 'Sort by ID'); ?>"></i> </a>
                </li>
            <?php else: ?>
                <li>
                    <a href="javascript:void(0);" onclick="$.ajax({data:{lastactivity:'start'}, success: function(){location.reload();}});" id="start-last">
                        <i class="fa-solid fa-arrow-down-wide-short fa-xl" title="<?php echo Yii::t('main-ui', 'Sort by last activity'); ?>"></i> </a>
                    </li>
                <?php endif; ?>
                <?php if (Yii::app()->session['requestFixHeader'] == 1): ?>
                    <li>
                        <a href="javascript:void(0);" onclick="$.ajax({data:{fixheader:'stop'}, success: function(){location.reload();}});" id="unfix-header">
                            <i class="fa-solid fa-window-maximize fa-xl" title="<?php echo Yii::t('main-ui', 'Open tickets in the same tab'); ?>"></i> </a>
                        </li>
                    <?php else: ?>
                        <li>
                            <a href="javascript:void(0);" onclick="$.ajax({data:{fixheader:'start'}, success: function(){location.reload();}});" id="fix-header">
                                <i class="fa-solid fa-up-right-from-square fa-xl" title="<?php echo Yii::t('main-ui', 'Open tickets in the new tab'); ?>"></i> </a>
                            </li>
                        <?php endif; ?>
                        <?php if (Yii::app()->session['requestResponsive'] == 1): ?>
                            <li>
                                <a href="javascript:void(0);" onclick="$.ajax({data:{responsive:'stop'}, success: function(){location.reload();}});" id="responsivetbl">
                                    <i class="fa-solid fa-table-cells-large fa-xl" title="<?php echo Yii::t('main-ui', 'Unset grid responsive'); ?>"></i> </a>
                                </li>
                            <?php else: ?>
                                <li>
                                    <a href="javascript:void(0);" onclick="$.ajax({data:{responsive:'start'}, success: function(){location.reload();}});" id="responsivetbl">
                                        <i class="fa-regular fa-rectangle-list fa-xl" title="<?php echo Yii::t('main-ui', 'Set grid responsive'); ?>"></i> </a>
                                    </li>
                                <?php endif; ?>
...
<!--                                   </li>-->
<!--                               --><?php //endif; ?>
                               <li><a href="javascript:void(0);" id="request-grid-full-ecolumns-dlg-link"><i class="fa-solid fa-gear fa-xl"
                                    ...
                                    if (Yii::app()->user->checkAccess('batchDeleteRequest')) {
                                        Yii::app()->clientScript->registerScript('delete', '
                                         $("#delete").click(function(){
                                             var checked=$("#request-grid-full").yiiGridView("getChecked","request-grid-full_c0"); // _c0 means the checkboxes are located in the first column, change if you put the checkboxes somewhere else
                                                    ...
                                                    swal({
                                                        ...
                                                        if (result.value) {
                                                            $.ajax({
                                                                ...
                                                                success:function(data){$("#request-grid-full").yiiGridView("update",{});},
                                                            });
                                                        }
                                                    });
                                                }
                                    ...
                                    if (Yii::app()->user->checkAccess('batchUpdateRequest')) {
                                        Yii::app()->clientScript->registerScript('close', '
                                         $("#close").click(function(){
                                             var checked=$("#request-grid-full").yiiGridView("getChecked","request-grid-full_c0");
                                             var count=checked.length;
                                             if(count==0){
                                                swal(
                                                    " '. Yii::t('main-ui', 'No items selected or selected only one item') . '",
                                                        ...
                                                        $("#btnSetStatus").click(function(){
                                                            ...
                                                            if($("#Status").find("option:selected").data("need_comment") == 1) {
                                                                ...
                                                                $.ajax({
                                                                    ...
                                                                    success:function (e) {
                                                                        jQuery("#myModalSetStatus").modal("toggle");
                                                                        $("#request-grid-full").yiiGridView("update",{});
                                                                    },
                                                                });
                                                            }
...
<?php if (Yii::app()->session['requestStopTimer'] == 1) {
    ...
    Yii::app()->clientScript->registerScript('autoupdate-activations-application-grid',
        "setInterval(function(){;$.fn.yiiGridView.update('request-grid-full');
