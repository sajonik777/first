    ...
    foreach ($fields as $field) {
      //$required = $field->req ? ' <span class="required"> *</span>' : '';
        if ($field->type == 'textFieldRow') {
            ...
            <input class="span12" maxlength="500" name="Companies[' . $field->id . ']" id="Companies_' . $field->id . '" type="text" value="' . $field->value . '"></div>';
        }
        if ($field->type == 'date') {
        ...
        if ($field->type == 'toggle') {
            ...
            $this->widget(
                ...
                array('name' => "Companies[" . $field->id . "]", 'value' => $field->value)
            );
            echo '</div>';
        ...
        if ($field->type == 'select') {
            $field_names = CompanyFieldset::model()->findByAttributes(array('id' => $field->fid));
            ...
            echo CHtml::DropDownList("Companies[" . $field->id . "]", $field->value,
                $arr,
                array(
                    'name' => "Companies[" . $field->id . "]",
                    'class' => 'span12',
