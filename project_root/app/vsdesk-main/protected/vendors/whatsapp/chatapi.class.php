...
class ChatApi
{
    ...
    public function getFullInbox()
    {
        ...
        while (true) {
            $inb = json_decode($this->query('messages',
                isset($offset) ? ['lastMessageNumber' => $offset] : ['last' => 1], "GET"), true);
    ...
    public function createInstance()
    {
        ...
        return json_decode(file_get_contents($this->instance_url . 'newInstance', false,
            isset($options) ? $options : null), true);
    }
    ...
    public function deleteInstance(int $id)
    {
        ...
        return json_decode(file_get_contents($this->instance_url . 'deleteInstance', false,
            isset($options) ? $options : null), true);
    }
    ...
    public function getInbox($offset = 0)
    {
        ...
        $inbox = json_decode($this->query('messages', ($offset > 0) ? ['lastMessageNumber' => $offset] : ['last' => 1]),
            1);
    ...
    {
        return json_decode($this->query('webhook', ['webhookUrl' => $url]), true);
    }
    ...
    {
        return json_decode($this->query('logout', []), true);
    }
    ...
    {
        return json_decode($this->query('reboot', []), true);
    }
    ...
    {
        return json_decode($this->query('sendFile', ['chatId' => $chat, 'filename' => $filename, 'body' => $body]),
            1)['sent'];
    }
     ...
     * @return bool
     */
    public function sendReadChat(string $chat): bool
    {
        return json_decode($this->query('readChat', ['chatId' => $chat]), 1)['read'];
    }
}
