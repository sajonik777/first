...
class AttributesMapBehavior extends CActiveRecordBehavior
{
    ...
    {
        if (in_array(Yii::app()->getComponent('mailqueue')->modelMap['MailQueue']['fieldAttachs'], $this->getOwner()->attributeNames())) {
            $this->getOwner()->{Yii::app()->getComponent('mailqueue')->modelMap['MailQueue']['fieldAttachs']} = CJSON::encode($value);
        }
    }
    ...
    {
        if (in_array(Yii::app()->getComponent('mailqueue')->modelMap['MailQueue']['fieldAttachs'], $this->getOwner()->attributeNames())) {
            $value = CJSON::decode($this->getOwner()->{Yii::app()->getComponent('mailqueue')->modelMap['MailQueue']['fieldAttachs']});
            if (null === $value) {
                return [];
            }
